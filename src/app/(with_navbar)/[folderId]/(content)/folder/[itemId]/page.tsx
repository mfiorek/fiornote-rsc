...
export default async function FolderPage({ params }: { params: { folderId: string; itemId: string } }) {
  ...
  const currentFolder = foldersData.find((folder) => folder.id === params.itemId);
  if (!currentFolder) {
    redirect('/404');
  }
    ...
    <>
      ...
      <div id='rightFolderFolders' className='flex w-full flex-col gap-2 empty:hidden'>
        {foldersData
          ...
          .map((folder) => (
            <ContentLink key={folder.id} href={`/${currentFolder.id}/folder/${folder.id}`} text={folder.name} variant='folder' />
          ))}
      </div>
      <div id='rightFolderNotes' className='flex w-full flex-col gap-2 empty:hidden'>
        {notesData
          ...
          .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())
          .map((note) => (
            <ContentLink key={note.id} href={`/${currentFolder.id}/note/${note.id}`} text={note.name} variant='note' />
          ))}
      </div>
    </>
  );
